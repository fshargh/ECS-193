...
from ecs193.settings import MEDIA_ROOT, MEDIA_URL
...
def profile(request, pk):
    ...
    if request.method == "POST":
        ...
        if pf.is_valid():
            ...
            im = PImage.open(imfn)
            im.thumbnail((193,193), PImage.ANTIALIAS)
            im.save(imfn, "JPEG")
    else:
...
def new_thread(request, pk):
    if request.method == 'POST':
        ...
        if(request.FILES):
            ...
            thread = Thread.objects.create(forum=forum, title=request.POST["subject"], creator=request.user)
...
def reply(request, pk):
    ...
    send_mail("new reply", "new reply on forum", 'chriscraftecs193@gmail.com', [thread.creator.email])
