...
def sending_mail( minutes ):
  for user in User.objects.all():
    if (user.emailEvery == minutes):
      ...
      msg = render_to_string('stebbins/periodic_template.html', {'date': datetime.now(), 'often': minutes, 'name': user.firstName, 'message': content})
      send_mail('Periodic Email', 'nothing', 'chriscraftecs193@gmail.com', [user.email], fail_silently=False, html_message= msg)
