...
media = '/home/chriscraft/ECS160WebServer/ecs160/media/'
...
def profile(request):
    ...
    return render(request, 'warcraft/profile.html', {'full_name': fname+" "+lname, 'avatar':picture, 'wins': wins, 'losses': losses, 'rating': rating, 'ranking': ranking})
...
def edit_profile(request):
    ...
    return render(request, 'warcraft/edit_profile.html', {'form': form, 'friendsname': friendsname})
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            if User.objects.filter(userName=request.POST['name']):
                ...
                return render(request, 'warcraft/friends.html', {'form': form, 'message': message})
